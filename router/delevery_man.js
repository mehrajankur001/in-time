...
router.get('/:id/editPassword', checkCookie, userAuth, checkRole(['admin', 'delevery-man']), async (req, res) => {
    try {
        const user = await User.findById(req.params.id);
        res.status(200).render(`users/delevery-man/editPassword`, { user: user });
    } catch (error) {
        res.json({
            message: 'Oh No'
        })
...
router.put('/:id/updatePassword', checkCookie, userAuth, checkRole(['admin', 'delevery-man']), async (req, res) => {
    try {
